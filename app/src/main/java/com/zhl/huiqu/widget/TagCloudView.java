...
public class TagCloudView extends ViewGroup {
    ...
    private static final int DEFAULT_TEXT_BACKGROUND = R.drawable.bg_search_tag;
    private static final int DEFAULT_VIEW_BORDER = Utils.dip2px(GlobalContext.getInstance(), 6);
    private static final int DEFAULT_TEXT_BORDER_HORIZONTAL = Utils.dip2px(GlobalContext.getInstance(), 0);
    private static final int DEFAULT_TEXT_BORDER_VERTICAL = Utils.dip2px(GlobalContext.getInstance(), 5);

    private static final int DEFAULT_TAG_RESID = R.layout.item_tag;
    private static final int DEFAULT_RIGHT_IMAGE = R.drawable.login_arrow_right;
    ...
    private int getMultiTotalHeight(int totalWidth, int totalHeight) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            // + marginLeft 保证最右侧 ViewGroup有边框有边界
    ...
    public void setTags(List<String> tagList) {
        ...
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
                ...
                LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, Utils.dip2px(GlobalContext.getInstance(), 30));
